...
UDPClientServer::UDPClientServer(QObject *parent)
  ...
  connect(m_socket, &QUdpSocket::readyRead, this,
          &UDPClientServer::onReadyRead);
}

UDPClientServer::~UDPClientServer() { unbind(); }
...
bool UDPClientServer::bind(quint16 port) {
  ...
  // 绑定到指定端口，接收所有网络接口的数据
  // 使用 ShareAddress 和 ReuseAddressHint
  // 允许多个程序绑定同一端口（用于广播接收）
  if (!m_socket->bind(QHostAddress::Any, port,
                      QAbstractSocket::ShareAddress |
                          QAbstractSocket::ReuseAddressHint)) {
    emit errorOccurred(
        QString("绑定端口失败: %1").arg(m_socket->errorString()));
    return false;
  }
...
void UDPClientServer::sendMessage(const QString &message,
  ...
  if (sent == -1) {
    ...
    emit errorOccurred(QString("发送不完整: 发送%1字节，实际%2字节")
                           .arg(data.size())
                           .arg(sent));
  } else {
...
void UDPClientServer::sendBroadcast(const QString &message,
                                    quint16 targetPort) {
  QByteArray data = message.toUtf8();
  ...
  qint64 sent =
      m_socket->writeDatagram(data, QHostAddress::Broadcast, targetPort);
  if (sent == -1) {
    emit errorOccurred(QString("广播失败: %1").arg(m_socket->errorString()));
  } else if (sent != data.size()) {
    emit errorOccurred(QString("广播不完整: 发送%1字节，实际%2字节")
                           .arg(data.size())
                           .arg(sent));
  } else {
    qDebug() << "UDP广播成功 -> 端口:" << targetPort << "内容:" << message
             << "(字节数:" << data.size() << ")";
  }
}
...
quint16 UDPClientServer::localPort() const { return m_socket->localPort(); }
